    ...
    def _serialize_bounding_box(self, model, node):
        ...
        if model._user_bounding_box is not None:
            if minversion("astropy", "5.0.5"):
                self._serialize_bounding_box_converter(model, node)
            else:
                self._serialize_bounding_box_legacy(model, node)

    def _serialize_bounding_box_legacy(self, model, node):
        ...
        if isinstance(bbox, ModelBoundingBox):
            ...
            bbox = CompoundBoundingBox.validate(model, bbox)

        node["bounding_box"] = bbox

    def _serialize_bounding_box_converter(self, model, node):
        from astropy.modeling.bounding_box import CompoundBoundingBox, ModelBoundingBox

        bbox = model.bounding_box

        # Run through the validate to change type from abstract bounding_boxes
        # generated by Model metaclass
        if isinstance(bbox, ModelBoundingBox):
