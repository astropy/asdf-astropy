...
__all__ = ['AstropyType', 'AstropyAsdfType', 'AsdfFormatType']
...
class AstropyTypeMeta(ExtensionTypeMeta):
    ...
    def __new__(mcls, name, bases, attrs):
        ...
        if cls.__name__ not in _TYPE_BASE_CLASS_NAMES:
            ...
            elif cls.organization == 'astropy-format.org' and cls.standard == 'asdf-transform-schemas':
                _asdf_format_types.add(cls)
...
class AsdfFormatType(CustomType, metaclass=AstropyTypeMeta):
    """
    This class represents types that have schemas that are defined in the ASDF
    standard (asdf-transforms) and  have tags that are implemented within asdf-astropy.

    """
    organization = 'astropy-format.org'
    standard = 'asdf-transforms'
